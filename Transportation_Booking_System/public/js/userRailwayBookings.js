...
async function getBookings() {
    try {
        ...
        const res = await axios.get('/get/getrailwaybookings', { headers: { "Authorization": token } });
        ...
        if (res.status === 200) {
           ...
           <h1>Hello, ${username}!</h1> <h2> Here are your Railways  booking details, payment methods, and more:</h2> 
           If you find any wrong data in your bookings, please update it or delete the booking.<br>

           Also, remember to add new bookings before your Railway time expires to ensure your seat is reserved.<br>
           ...
           Feel free to explore and manage your bookings. Enjoy your journey!
            ...
            getBookings.forEach((booking, index) => {
                ...
                async function deleteBooking(bookingId) {
                    try {
                        const token = localStorage.getItem("token");
                        const res = await axios.delete(`/railwaydelete/trainbooking/${bookingId}`, { headers: { "Authorization": token } });
                        console.log('1===?')
                        if (res.status === 200) {
                            console.log('===>2')
