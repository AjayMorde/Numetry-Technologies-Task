...
async function getBookings() {
    try {
        ...
        const res = await axios.get('/get/getbookings', { headers: { "Authorization": token } });
        ...
        if (res.status === 200) {
           ...
           <h1>Hello, ${username}!</h1> <h2> Here are your Bus booking details, payment methods, and more:</h2> 
           ...
           Feel free to explore and manage your bookings. Enjoy your journey!
            ...
            getBookings.forEach((booking, index) => {
                ...
                async function deleteBooking(bookingId) {
                    try {
                        const token = localStorage.getItem("token");
                        const res = await axios.delete(`/delete/booking/${bookingId}`, { headers: { "Authorization": token } });
                        console.log('1===?')
                        if (res.status === 200) {
                            console.log('===>2')
