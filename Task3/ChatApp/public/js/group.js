...
async function createGroup() {
  try {
    ...
    const res = await axios.post(
      "http://localhost:4500/group/createGroup",
      {
        groupName: groupName,
        members: members,
      },
...
async function getGroups() {
  try {
    const token = localStorage.getItem("token");
    const res = await axios.get("http://localhost:4500/group/getGroups", {
      headers: { Authorization: token },
    });
    getGroups.innerHTML = "";
...
async function addToGroup() {
  try {
    ...
    const token = localStorage.getItem("token");
    const res = await axios.post(
      "http://localhost:4500/group/addToGroup",
      {
        groupName: groupName,
        members: members,
      },
...
async function deleteFromGroup() {
  try {
    ...
    const token = localStorage.getItem("token");
    const res = await axios.post(
      "http://localhost:4500/group/deleteFromGroup",
      {
        groupName: groupName,
        members: members,
      },
...
async function groupMembers() {
  try {
    ...
    const res = await axios.get(
      `http://localhost:4500/group/groupMembers/${groupName}`,
      {
        headers: { Authorization: token },
      }
    );
...
groupMembersBtn.addEventListener("click", groupMembers);    YES